...
def seed_daily(repos, *, day: date = DAY, equity: float = EQUITY) -> None:
        ...
        [
            {
                ...
                "slippage": -1.1,
                "net_pnl": -33.3,
                "traded_notional": 1500.0,
                "signal": 0.4,
            },
            {
                "symbol": "ETHUSDT",
                "side": "short",
                "avg_notional": -2073.561,
                "price_pnl": -25.9,
                "funding": 2.0,
                "fees": -3.0,
                "slippage": -1.0,
                "net_pnl": -27.9,
                "traded_notional": 1620.0,
                "signal": -0.6,
            },
        ],
    )
    ...
    repos.universe.save(
        UniverseResult(
            ...
            entries=tuple(
                UniverseEntry(
                    symbol=f"SYM{i}USDT", rank=i, median_quote_volume_30d=1e8, history_days=500, included=True
                )
                for i in range(16)
            ),
        ),
    ...
    repos.state.save(
        ...
        governor_g=1.0,
        blocks=[],
        context={"backup_lag_s": 8.0, "bnb_days": 41.3},
        now_ms=end_ms,
    )
...
def _book(equity: float) -> list[Position]:
    ...
    positions = [Position(symbol="ETHUSDT", qty=-1.0, entry_price=2100.0, mark_price=0.21 * equity)]
    for i in range(6):
        positions.append(Position(symbol=f"L{i}USDT", qty=1.0, entry_price=100.0, mark_price=0.05 * equity))
    for i in range(8):
        positions.append(Position(symbol=f"S{i}USDT", qty=-1.0, entry_price=100.0, mark_price=0.04 * equity))
    return positions
